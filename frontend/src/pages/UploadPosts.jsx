...
const UploadPosts = () => {
  ...
  const handleDropdownImageChange = (index, files) => {
    ...
    updated[index].dropdownImages = [...updated[index].dropdownImages, newImageUrl];
    setDropdowns(updated);
  ...
  return (
    <div className="flex items-center justify-center p-10 gap-2">
      <div className="bg-[#212121] p-6 rounded-lg w-full max-w-md shadow-lg">
          ...
          {dropdowns.map((dropdown, index) => (
            <div key={index} className="border-t pt-4 border-white">
              ...
              <input
                ...
                onChange={(e) => handleDropdownChange(index, "subtitle", e.target.value)}
              />
              ...
              >
                {dropdown.dropdownImages.length > 0 ? (
                  <div className="relative w-full mx-auto overflow-hidden">
                    ...
                    {dropdown.dropdownImages.length > 1 && currentImageIndices[index] > 0 && (
                      <button
                        type="button"
                    ...
                    {dropdown.dropdownImages.length > 1 &&
                      currentImageIndices[index] < dropdown.dropdownImages.length - 1 && (
                        <button
                          type="button"
                      ...
                      {dropdown.dropdownImages.map((_, idx) => (
                        <div
                          ...
                          className={`w-2 h-2 rounded-full cursor-pointer transition-all duration-300 ease-in-out ${
                            currentImageIndices[index] === idx ? "bg-white" : "bg-gray-500"
                          }`}
                          onClick={(e) => {
                            e.preventDefault();
                            setCurrentImageIndices((prev) => ({
                ...
                <input
                  ...
                  onChange={(e) => handleDropdownImageChange(index, e.target.files)}
                  className="hidden"
                />
              </label>
              ...
              <textarea
                ...
                onChange={(e) => handleDropdownChange(index, "description", e.target.value)}
              ></textarea>
            </div>
          ))}
        </div>
        ...
        <div>
          <button className="text-white bg-green-500 w-full p-2 font-medium text-xl cursor-pointer rounded hover:bg-green-600">
            Submit
          </button>
        </div>
      </div>

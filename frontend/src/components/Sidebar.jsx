      ...
      <div className="inline-block min-h-screen border-r-[#262626] border-r-1 w-[250px] max-sm:w-[70px]">
        <ul className="flex flex-col items-start p-2 space-y-2">
          <div className="flex items-center p-3 max-sm:p-0 max-sm:flex max-sm:justify-center gap-2 w-full mt-6 mb-2 h-20">
            <img src={assets.logo} alt="" className="w-10 h-8 sm:hidden rounded cursor-pointer" />
            <p className="max-sm:hidden pl-12 font-serif italic text-2xl cursor-pointer text-white">Tivana</p>
          </div>
          ...
          {/* Search Div */}
          <div onClick={handleSearchClick} className={`flex items-center p-3 gap-2 w-full hover:bg-[#2e2d2d] cursor-pointer rounded ${activeNavLink === "search" ? "text-[#32CD32] font-semibold" : "text-white"}`}>
            <img src={activeNavLink === "search" ? assets.searchFilled : assets.search} alt="" className="w-6 h-6"/>
            <p className="max-sm:hidden ml-2">Search</p>
          </div>

          {/* Explore Div */}
          <div className={`flex items-center p-3 gap-2 w-full hover:bg-[#2e2d2d] rounded cursor-pointer ${activeNavLink === "explore" ? "text-[#32CD32] font-semibold" : "text-white"}`} onClick={() => handleNavLinkClick("explore", "/explore")}>
            <img src={activeNavLink === "explore" ? assets.compassFilled : assets.compass} alt="" className="w-6 h-6"/>
            <p className="max-sm:hidden ml-2">Explore</p>
          </div>

          {/* Messages Div */}
          <div className={`flex items-center p-3 gap-2 w-full hover:bg-[#2e2d2d] rounded cursor-pointer ${activeNavLink === "messages" ? "text-[#32CD32] font-semibold" : "text-white"}`} onClick={() => handleNavLinkClick("messages", "/messages")}>
            <img src={activeNavLink === "messages" ? assets.chatFilled : assets.chat} alt="" className="w-6 h-6"/>
            <p className="max-sm:hidden ml-2">Messages</p>
          </div>

          {/* Notifications Div */}
          <div onClick={handleNotificationsClick} className={`flex items-center p-3 gap-2 w-full hover:bg-[#2e2d2d] cursor-pointer rounded ${activeNavLink === "notifications" ? "text-[#32CD32] font-semibold" : "text-white"}`}>
            <img src={activeNavLink === "notifications" ? assets.heartFilled : assets.heart} alt="" className="w-6 h-6"/>
          ...
          {/* Menu Div */}
          <div onClick={handleMenuClick} className={`flex items-center p-3 gap-2 w-full hover:bg-[#2e2d2d] cursor-pointer rounded mt-16 ${activeNavLink === "menu" ? "text-[#32CD32] font-semibold" : "text-white"}`}>
            <img src={activeNavLink === "menu" ? assets.menuFilled : assets.menu} alt="" className="w-6 h-6"/>
